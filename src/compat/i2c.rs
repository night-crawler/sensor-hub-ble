...
pub trait I2CWrapper<E> {
    async fn write_read(&mut self, address: u8, wr_buffer: &[u8], rd_buffer: &mut [u8]) -> Result<(), E>;
    async fn write(&mut self, address: u8, buffer: &[u8]) -> Result<(), E>;
    async fn read(&mut self, address: u8, buffer: &mut [u8]) -> Result<(), E>;
}

impl<'a, T: twim::Instance> I2CWrapper<CustomI2CError> for Twim<'a, T> {
